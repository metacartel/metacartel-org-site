  ...
  return new Promise(async (resolve, reject) => {
    try {
      const response: Response = await fetch(
        `https://api.twitter.com/2/users/${metaCartelTwitterAccount}/tweets?max_results=${maxResults}&tweet.fields=attachments%2Cin_reply_to_user_id&media.fields=preview_image_url%2Curl&expansions=attachments.media_keys%2Cin_reply_to_user_id&user.fields=name`,
        requestOptions
      );
